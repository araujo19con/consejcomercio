    ...
    <Dialog open onOpenChange={open => !open && onClose()}>
      <DialogContent className="max-w-2xl p-0 flex flex-col max-h-[90vh] overflow-hidden gap-0">
        <div className="px-6 py-4 border-b shrink-0">
          <h2 className="text-lg font-semibold text-foreground">Novo Contrato — Oportunidade Convertida</h2>
          <p className="text-sm text-muted-foreground">{op.titulo} · {op.cliente?.nome}</p>
        </div>
        ...
        <div className="overflow-y-auto p-6 space-y-5 flex-1">
          ...
          {/* Áreas de direito */}
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-2">Áreas de Direito</label>
        ...
        <div className="flex gap-3 px-6 py-4 border-t shrink-0">
          <button onClick={onClose} className="flex-1 px-4 py-2 border rounded-lg text-sm text-muted-foreground hover:bg-background">
            Cancelar
          </button>
    ...
    <Dialog open onOpenChange={open => !open && onClose()}>
      <DialogContent className="max-w-md p-0 gap-0">
        <div className="px-6 py-4 border-b">
          <h2 className="text-lg font-semibold text-foreground">Nova Oportunidade</h2>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Data de Alerta</label>
        ...
        <div className="flex gap-3 px-6 py-4 border-t">
          <button onClick={onClose} className="flex-1 px-4 py-2 border rounded-lg text-sm text-muted-foreground hover:bg-background">Cancelar</button>
            ...
            return (
              <div key={statusInfo.value} className="w-72 shrink-0">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-fg2">{statusInfo.label}</h3>
                      ...
                      <Card key={op.id} className="shadow-sm">
                        <CardContent className="p-3">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            {op.valor_estimado && <span className="text-xs font-semibold text-fg2">{formatCurrency(op.valor_estimado)}</span>}
                          </div>
                          <p className="text-sm font-medium text-foreground leading-tight">{op.titulo}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">{op.cliente?.nome}</p>
